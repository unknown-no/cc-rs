...
impl Build {
    ...
    pub fn get_canonical_library_names<'a>(
        &self,
        output: &'a str,
    ...
    pub fn try_compile(&self, output: &str) -> Result<(), Error> {
        ...
        if self.link_shared_flag {
            ...
            cmd.args(["-shared", "-o"])
                .arg(self.get_shared_lib_out_dir()?.join(dynlib_name))
                .args(&objects);
    ...
    fn get_shared_lib_out_dir(&self) -> Result<Cow<'_, Path>, Error> {
        match &self.shared_lib_out_dir {
            Some(p) => Ok(Cow::Borrowed(&**p)),
            None => self.get_out_dir(),
        }
    }
